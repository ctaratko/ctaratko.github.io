...
files <- list.files(path = "/Users/christophertaratko/ProjectsList/Data Analytics/DANL 310 Project/Data Analysis", pattern = "*.csv", full.names = TRUE)
...
#ID_freq_comp = read.csv("Data Analysis/Proportion_of_Fill_By_Location - Sheet1.csv")
ID_freq_comp = read.csv("Data Analysis/Proportion_of_Fill_By_Location - Sheet1.csv")
...
```{r}
#Descriptive Stats: Number of Major Fill Types
...
# By Year
# 1. Total per Year
year_totals <- combined_df_no_na |>
  group_by(Year) |>
  summarise(
    total_filled = sum(Number_of_Tubes_First_Filled),
    .groups = "drop"
  )

# 2. Totals per Year + Fill_Type
type_totals <- combined_df_no_na |>
  group_by(Year, Fill_Type) |>
  summarise(
    type_total = sum(Number_of_Tubes_First_Filled),
    .groups = "drop"
  )

# 3. Join + compute percentages
stats_by_year <- type_totals |>
  left_join(year_totals, by = "Year") |>
  mutate(
    percent = type_total / total_filled
  )
...
standardize <- function(df, year_label) {

  df <- clean_names(df)

  # Identify possible fill column
  fill_col <- names(df)[str_detect(names(df), "fill")][1]

  # Only rename IF needed
  if (fill_col != "number_of_tubes_first_filled") {
    df <- df %>%
      rename(number_of_tubes_first_filled = all_of(fill_col))
  }

  # Same logic for week column
  week_col <- names(df)[str_detect(names(df), "week")][1]

  if (week_col != "week") {
    df <- df %>%
      rename(week = all_of(week_col))
  }
  ...
  df %>%
    mutate(
      ...
      number_of_tubes_first_filled =
        replace_na(number_of_tubes_first_filled, 0)
    ) %>%
    select(year, week, number_of_tubes_first_filled)
}
```
...
#Create cummulative for each year
